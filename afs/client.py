...
class EIPaaSAFSSession:
    ...
    def _get_attributes(self):
        ...
        attributes = {key: value for key, value in self.__dict__.items() if not callable(value) and key not in exclude_keys}
        return attributes
    ...
    def login(self, target_endpoint, username, password):
        ...
        resp = requests.get(
            urljoin(self.target_endpoint, 'info')
        )
        ...
        if not resp.ok:
            message = {
                'error': 'Login to {0} as user {1} failed'.format(self.target_endpoint, username)
            }
            raise Exception(message)
    ...
    def list_service_instances(self):
        ...
        resp = requests.get(
            urljoin(self.target_endpoint, str(Path(self.api_version).joinpath('instances'))),
        ...
        if not resp.ok:
            message = {
                'error': 'List service instances failed',
                'response': resp.text
            }
            raise Exception(message)
    ...
    def target(self, service_instance_id=None):
        ...
        if not service_instance_id:
            ...
            if not service_instance_id:
                message = {
                    'error': 'Please login adn select service instance first'
                }
                raise Exception(message)
        ...
        resp = requests.get(
            urljoin(self.target_endpoint, str(Path(self.api_version).joinpath(service_instance_id))),
            headers={'Authorization': 'Bearer ' + self._token},
            timeout=self.timeout
        )
        ...
        if not resp.ok:
            message = {
                'error': 'Get info of service instance {0} failed'.format(service_instance_id),
                'response': resp.text
            }
            raise Exception(message)
    ...
    def push(self, source_path='./', manifest_path='./manifest.yml', name=None):
        ...
        if not workspace_endpoint or not auth_code:
            message = {
                'error': 'Please login and select your AFS instance first'
            }
            raise Exception(message)
        ...
        if not manifest_path.is_file() and manifest_path != source_path.joinpath('manifest.yml'):
            message = {
                'warning': 'Manifest path {0} is not a file, use {1} as manifest'.format(manifest_path, source_path.joinpath('manifest.yml'))
            }
        ...
        if not manifest_path.is_file():
            message = {
                'error': 'Manifest path {0} is not file'
            }
            raise Exception(message)
        ...
        try:
            ...
            message = {
                'error': 'Manifest is not valid yaml file'
            }
            raise Exception(message)
        ...
        if not name:
            try:
                ...
                message = {
                    'error': 'Manifest seems not a valid cloud foundry manifest'
                }
                raise Exception(message)

        if not name:
            message = {
                'error': 'Please provide name of your analytic app'
            }
            raise Exception(message)
        ...
        resp = requests.get(
            urljoin(self.target_endpoint, str(Path(workspace_endpoint).joinpath('analytics'))),
        ...
        if resp.ok:
            ...
            if resp:
                message = {
                    'message': 'Find analytic app {0}, update now'.format(name)
                }
                print(message)
                push_url = Path(workspace_endpoint).joinpath('analytics', resp[0]['uuid'])
            else:
                message = {
        ...
        with ZipFile(archive, mode='w', compression=ZIP_DEFLATED) as zip_file:
            for root, dirs, files in os.walk(source_path):
                for file in files:
                    ...
                    zip_file.write(Path(root).joinpath(file), arcname=Path(path).joinpath(file))

        archive.seek(0)
        files = {
            'archive': ('archive.zip', archive, 'application/zip')
        }
        resp = requests.put(
